...
def date_preparser(candidates):
    """
    Takes ambiguous dates, determines to which key or keys the data needs to
    be translated and returns a dict of data, with as a key the translation
    function that needs to be used.
    """
    dtype_mapping = trl_rules['_general']['datetype_mapping']

    def handle_dict(dict_, dtype_value_dict):
        """
        Pre-parses date information in dicts
        """
        # Check if the date type can be found
        datetype = None
        for key in trl_rules['_general']['datetype_keys']:
            if key in dict_:
                data = dict_[key]
                ...
                if org_typename is not None and org_typename in dtype_mapping:
                    datetype = dtype_mapping[org_typename]
                    break
        else:
            return {}  # No datetype found, so stop

        # Check if a date can be extracted
        for key in trl_rules['_general']['datevalue_keys']:
            if key in dict_:
                data = dict_[key]
                if isinstance(data, str):
                    ...
                    if '/' in data and org_typename == "collected":
                        ...
                        if len(parts) == 2 and len(parts[0]) == len(parts[1]):
                            dtype_value_dict['timePeriod'] = {'temporal': data}
                            return
        ...
        if datetype == 'otherDates':
            def_key = org_typename
        if datetype in dtype_value_dict:
            if datetype == 'modified':
                if date > dtype_value_dict[datetype][def_key]:
                    dtype_value_dict[datetype][def_key] = date
            else:
                if date < dtype_value_dict[datetype][def_key]:
                    dtype_value_dict[datetype][def_key] = date
        else:
    ...
    translate_to = {}
    for ckey, cdata in candidates.items():
        if isinstance(cdata, dict):
            handle_dict(cdata, translate_to)
        elif isinstance(cdata, list):
            for item in cdata:
                if isinstance(item, dict):
                    handle_dict(item, translate_to)

    return translate_to


def title(candidates):
    """
    Generates the title, based on a dict of original key/value combination
    candidates
    """
    rules = trl_rules['title']

    def convert_str(str_):
        title = _aux.filter_truncate_string(
            str_,
            rules['length']['min'],
            rules['length']['max']
        )
        ...
        if 'PT_FreeText' in dict_:
            ...
            if isinstance(language_options, list):
                ...
                if langval is not None:
                    title = convert_str(langval)
                    if title is not None:
                        return title

        for dkey in rules['dict_key_priority']:
            value = dict_.pop(dkey, None)
            if isinstance(value, str):
                title = convert_str(value)
                if title is not None:
                    return title
            if isinstance(value, dict):
                title = convert_dict(value)
                if title is not None:
                    return title

        # If priority dict key not found, try all others
        for dkey, value in dict_.items():
            if dkey not in rules['dict_keys_type'] and isinstance(value, str):
                title = convert_str(value)
                if title is not None:
                    return title
    ...
    def convert_list(list_):
        ...
        for item in list_:
            ...
            if isinstance(item, str):
                c_title = convert_str(item)
            elif isinstance(item, dict):
                c_title = convert_dict(item)
                c_title_type = _get_value(
                    item, rules['dict_keys_type'], value_type=str
                )
                if c_title_type in rules['type_priority']:
                    c_title_prio = rules['type_priority'].index(c_title_type)
    ...
    def convert_title_data(title_data):
        "Converts the different data types to a string in the correct format"
        title = None
        if isinstance(title_data, str):
            title = convert_str(title_data)
        elif isinstance(title_data, dict):
            title = convert_dict(title_data)
        elif isinstance(title_data, list):
            title = convert_list(title_data)

        return title

    title = None
    for key in rules['data_priority']:
        title_data = candidates.pop(key, None)
        if title_data is not None:
            title = convert_title_data(title_data)
            if title is not None:
                break
    else:
        for key, value in candidates.items():
            title = convert_title_data(value)
            if title is not None:
                break
